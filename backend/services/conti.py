...
def aggiungi_conto(conto: CreaConto, session: Session) -> Conto | None:
    try:
        id_utente = get_user_id_from_uuid(conto.uuid_utente, session)
        if id_utente is None:
            raise ValueError(f"L'utente con UUID {conto.uuid_utente} non è stato trovato.")
...
def chiusura_conto(uuid_conto: uuid.UUID, session: Session) -> Conto:
    try:
